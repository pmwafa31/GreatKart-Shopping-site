...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = AccountUser.objects.create_user(first_name=first_name, last_name=last_name, email=email, username=username,
                                               password=password)
...
def forgotpassword(request):
    if request.method == 'POST':
        ...
        if AccountUser.objects.filter(email=email).exists():
            user = AccountUser.objects.get(email__exact=email)
...
def change_password(request):
    if request.method == 'POST':
        ...
        user = AccountUser.objects.get(username__exact=request.user.username)
